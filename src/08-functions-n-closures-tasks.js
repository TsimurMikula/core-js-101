...
function getPolynom() {
  ...
  /* return function getMathFunc() {
    if (args.length === 3) return 'y = ' + args[0] + '*x^2 '+ args[1] + '*x + ' + args[2];
    if (args.length === 2) return `y = ${args[0]}*x + ${args[1]}`;
    if (args.length === 1) return `y = ${args[0]}`;
    return null;
  }; */
}
